...
class HandlerInvoker:
    ...
    def resolve_kwargs(hints: dict, request: Request) -> dict | Response:
        ...
        for param_name, class_reference in hints.items():
            if param_name in HandlerInvoker._IGNORE_PARAMS or (isinstance(class_reference, type) and issubclass(class_reference, Request)):
                continue

            if (HandlerInvoker.let_model_resolve(class_reference=class_reference, param_name=param_name, request=request, kwargs=kwargs)):
                continue

            HandlerInvoker.let_container_resolve(class_reference=class_reference, param_name=param_name, kwargs=kwargs)
